...
%每一列分布表示 当前工件工序下一个工序的索引，当前工序所在机器的下一个工序索引，
%当前工序工件的上一个工序的索引，当前工序所在机器的上一个工序的索引
...
e=0;
%采用广度遍历更新每个顶点的最早开始时间和最晚开始时间,
%如果不采用广度优先遍历，则会在更新后续层的时候，出现前序结点来不及计算，而导致计算错误。
...
for i=1:SH
    ...
    if digraph(Index,3)~=0
        ...
        t1=time{f_index,s1(last),s2(last),mm(last)}+VELTime{last,1};
    end
    
    if digraph(Index,4)~=0
        last=digraph(Index,4);
        if isempty(VELTime{last,1})
            VELTime{last,1}=CalculateLastOperation(s1,s2,mm,digraph,last,VELTime,f_index);
        end
        t2=time{f_index,s1(last),s2(last),mm(last)}+VELTime{last,1};
    end
    if t1>t2 %如果t1大于t2则
        VELTime{Index,1}=t1;
    else
    ...
    if digraph(Index,1)==0 %如果是工件的最后一个工序则说明下一个节点是LastNode虚拟结束节点, 则虚拟节点的上一个节点是所有工件的最后一个工序
        t1=time{f_index,s1(Index),s2(Index),mm(Index)}+VELTime{Index,1};%选最大的作为最后的虚拟节点
        if t1>LastNode
            LastNode=t1;
        end
    end
...
for i=SH:-1:1
    ...
    if digraph(Index,1)==0
        if digraph(Index,2)==0
            VELTime{Index,2}=LastNode-time{f_index,s1(Index),s2(Index),mm(Index)}; %最后一个拓扑有序的结点的工序的最晚开始时间等于最早开始时间
        else
            t1=LastNode-time{f_index,s1(Index),s2(Index),mm(Index)};
            next=digraph(Index,2);
            if isempty(VELTime{next,2})
                VELTime{next,2}=CalculateNextOperation(s1,s2,mm,digraph,next,VELTime,LastNode,f_index);
            end
            t2=VELTime{next,2}-time{f_index,s1(Index),s2(Index),mm(Index)};
            if t1>t2 %选最小的
                VELTime{Index,2}=t2;
            else
                VELTime{Index,2}=t1;
    ...
    if digraph(Index,1)~=0
        ...
        t1=VELTime{next,2}-time{f_index,s1(Index),s2(Index),mm(Index)};%下一个节点减当前这条边的时间，但是时间在节点上所有减当前节点是时间
        if digraph(Index,2)==0
            VELTime{Index,2}=t1;
            continue;
        end
...
if digraph(Index,3)~=0
    ...
    if isempty(VELTime{zlast,1})
        VELTime{zlast,1}=CalculateLastOperation(s1,s2,mm,digraph,zlast,VELTime,f_index);
    end
    t1=time{f_index,s1(zlast),s2(zlast),mm(zlast)}+VELTime{zlast,1};
end
...
if digraph(Index,4)~=0
    ...
    t2=time{f_index,s1(zlast),s2(zlast),mm(zlast)}+VELTime{zlast,1};
end
if t1>t2 %如果t1大于t2则
    VELTime{Index,1}=t1;
else
...
if digraph(Index,1)==0
    ...
    if isempty(VELTime{znext,1})
        VELTime{znext,1}=CalculateNextOperation(s1,s2,mm,digraph,znext,VELTime,f_index);
    end
    t1=VELTime{znext,1}-time{f_index,s1(Index),s2(Index),mm(Index)};
end

if digraph(Index,2)==0
    t2=LastNode-time{f_index,s1(Index),s2(Index),mm(Index)};
else
    znext=digraph(Index,2);
    if isempty(VELTime{znext,1})
        VELTime{znext,1}=CalculateNextOperation(s1,s2,mm,digraph,znext,VELTime,f_index);
    end
    t2=VELTime{znext,1}-time{f_index,s1(Index),s2(Index),mm(Index)};
end
if t1>t2 %如果t1大于t2则
    VELTime{Index,1}=t2;
else
